...
class StrategyProfile(object):
    ...
    def expected_value(self):
        ...
        for ev in expected_values:
            assert (len(ev) == 1)
        return tuple(next(ev.values()) for ev in expected_values)  # pull the EV from the dict returned
    ...
    #         prob = pathprobs / float(len(root.children))
