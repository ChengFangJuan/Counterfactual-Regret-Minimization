...
class PokerTrainer(object):
    ...
    def computer_possible_card(self):
        out = []
        for card1 in LEDUC_DECK:
            player0_card = card1
            left_card = copy.deepcopy(LEDUC_DECK)
            left_card.remove(player0_card)
            for card2 in left_card:
                player1_card = card2
                board_card = copy.deepcopy(left_card)
                board_card.remove(player1_card)
                for board in board_card:
                    temp_card = [player0_card, player1_card, board]
                    if temp_card not in out:
                        out.append(temp_card)
        print("----------- possible card -------")
        print(out)
        print(len(out))
        return out
    ...
    def train(self, iterations):
        ...
        for iter_count in range(1, iterations + 1):
            ...
            if iter_count % 10 == 0:
                ...
                strategy_delta.append(sum_delta_value / len(pre_strategy))
    ...
    def cfr(self, history, p0, p1, roundCounter, stage2):
        ...
        for i in range(actions):
            ...
            # utilities[i] = utilities[i] * (p1 if currentPlayer == 0 else p0)
